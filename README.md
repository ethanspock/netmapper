...
- Map: Network diagram; drag nodes (multi-select with Ctrl), zoom/pan, export, Save/Load Layout.
- Table: Host list and merged ports (Nmap + passive/imported).
...
- Import Capture: load .pcap/.pcapng/tcpdump text to populate the Table/Map.

Geolocation

- Select hosts on the map (Ctrl+click) or in the table and click “Geolocate Selection”.
- Uses ip-api.com (free tier) to resolve city/region/country/lat/lon/ISP (requires internet access).
- Results appear in the Listener Log and an info popup.
...
- Windows
  ...
  - Use Auto Detect, then Start Listen; if needed, switch to another NPF device.
